...
interface AadharResultsListProps {
  results: AadharData[]
  loading: boolean
  searchAttempted: boolean
}

export default function AadharResultsList({ results, loading, searchAttempted }: AadharResultsListProps) {
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-slate-600 dark:text-slate-400">Retrieving aadhar information</p>
      </div>
    )
  }
  ...
  if (!searchAttempted) {
    return (
      <div className="text-center py-16">
        <p className="text-slate-600 dark:text-slate-400">Search an Aadhar Number for Results</p>
      </div>
    )
  }
        ...
        {results.map((result, index) => (
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              <ResultField emoji="🏠" label="Address" value={result.address} />
              <ResultField emoji="📱" label="Mobile" value={result.aadhar} />
              <ResultField emoji="📞" label="Alt Mobile" value={result.alt || "N/A"} />
              <ResultField emoji="📧" label="Email" value="N/A" />
              <ResultField emoji="🆔" label="ID" value={result.id} />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex-1">
        ...
        <p className="text-slate-900 dark:text-white break-words">{value?.replace(/!/g, ", ") || "N/A"}</p>
      </div>
    </div>
  )
}
